                    ...
                    <Col md={4} style={colStyle} className="pl-4 pr-4 pt-4 pb-1 border rounded bg-light">
                        ...
                        >
                            <FormGroup row>
                                ...
                                <Col md={3}>
                                    <Input type='select' id='part' name='part' value={isAddTeam ? add.get('part') : edit.get('part')} onChange={onChange(isAddTeam ? 'add' : 'edit')}>
                                        <option>-- 공종 --</option>
                                        {
                                ...
                                <Col md={5}>
                                    <Input type='text' id='teamName' name='teamName' value={isAddTeam ? add.get('teamName') : edit.get('teamName')} onChange={onChange(isAddManager ? 'add' : 'edit')} />
                                </Col>
                            </FormGroup>
                            <FormGroup row>
                        ...
                        >
                            <FormGroup row>
                                ...
                                <Col md={4}>
                                    <Input type='text' id='name' name='name' value={isAddManager ? addManager.get('name') : editManager.get('name')} onChange={onChange(isAddManager ? 'addManager' : 'editManager')} />
                                </Col>
                                <Label md={2} for='position' className='text-right'>직책</Label>
                                <Col md={3}>
                                    <Input type='text' id='position' name='position' value={isAddManager ? addManager.get('position') : editManager.get('position')} onChange={onChange(isAddManager ? 'addManager' : 'editManager')} />
                                </Col>
                            </FormGroup>
                            <FormGroup row>
                                <Label md={3} for='effStaDt' className='text-right'>시작일</Label>
                                <Col md={9}>
                                    <Input type='date' id='effStaDt' name='effStaDt' value={isAddManager ? addManager.get('effStaDt') : editManager.get('effStaDt')} onChange={onChange(isAddManager ? 'addManager' : 'editManager')} />
                                </Col>
                            </FormGroup>
                            <FormGroup row>
                                <Label md={3} for='effEndDt' className='text-right'>종료일</Label>
                                <Col md={9}>
                                    <Input type='date' id='effEndDt' name='effEndDt' value={isAddManager ? addManager.get('effEndDt') : editManager.get('effEndDt')} onChange={onChange(isAddManager ? 'addManager' : 'editManager')} />
                                </Col>
                            </FormGroup>
                            <FormGroup row>
                                <Col className="d-flex align-items-center justify-content-end">
                                    {isAddManager ?
                                        (<Button color="primary" onClick={() => onSaveManager(team.get('_id'))}>담당자 추가</Button>) :
                                        (<React.Fragment>
