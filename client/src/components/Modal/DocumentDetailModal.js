    ...
    >
      ...
      <ModalHeader toggle={onClose("documentDetail")} className="bg-light">
        ...
        <span className="text-primary">
          ({data.get("documentNumber")}_{data.get("documentTitle")}_Rev.
          {data.get("documentRev")})
        </span>
      </ModalHeader>
        ...
        <Table borderless className="rounded m-0">
          ...
          <tbody>
            ...
            <tr className="border-bottom">
              ...
              <td>
                <div className="pb-1">
                  {data.getIn(["level", "description"])} (Max: 5)
                </div>
            ...
            <tr>
              ...
              <td colSpan="3">
                <span className="text-secondary">
                  등록: {data.getIn(["timestamp", "regId"])} (
                  {data.getIn(["timestamp", "regDt"])})
                </span>
                <br />
                <span className="text-secondary">
                  수정: {data.getIn(["timestamp", "updId"])} (
                  {data.getIn(["timestamp", "updDt"])})
                </span>
              </td>
            </tr>
          </tbody>
        ...
        {writable && [
            ...
            <InputGroup>
              ...
              >
                ...
                {codes.get("cdMinors").map(code => (
                  <option
                    key={code.get("_id")}
                    value={JSON.stringify(code.get("cdRef1"))}
                  >
                    {code.get("cdSName")}
                  </option>
                ))}
          ...
          <ButtonGroup key={3}>
            {data.getIn(["deleteYn", "yn"]) === "NO" ? (
              <Button
                color="danger"
                onClick={onDelete({ id: data.get("id"), yn: "YES" })}
              >
                삭제
              </Button>
            ) : (
              <Button
                color="danger"
                onClick={onDelete({ id: data.get("id"), yn: "NO" })}
              >
                삭제 취소
              </Button>
            )}
            {data.getIn(["holdYn", -1, "yn"]) === "NO" ? (
              <Button
                color="info"
                onClick={onHold({ id: data.get("id"), yn: "YES" })}
              >
                보류
              </Button>
            ) : (
              <Button
                color="info"
                onClick={onHold({ id: data.get("id"), yn: "NO" })}
              >
                보류 취소
              </Button>
            )}
